...
void print_op(Pipe_Op *op) {
	if (op)
		printf(
		    "OP (PC=%08x inst=%08x) src1=R%d (%08x) src2=R%d (%08x) dst=R%d valid "
		    "%d (%08x) br=%d taken=%d dest=%08x mem=%d addr=%08x\n",
		    op->pc, op->instruction, op->reg_src1, op->reg_src1_value, op->reg_src2,
		    op->reg_src2_value, op->reg_dst, op->reg_dst_value_ready,
		    op->reg_dst_value, op->is_branch, op->branch_taken, op->branch_dest,
...
void pipe_stage_mem() {
	...
	if (!ls_stall_counter && (data_cache->state == DELAY_MC_PIPE_MW || data_cache->state == DELAY_MC_PIPE_MR)) {
		...
		if (data_cache->state == DELAY_MC_PIPE_MR)
			...
			data_cache->state = WAIT_FILL_NOTIF; //TODO maybe NO_MISS
		return;
	}

	//if we are waiting for a fill notif, stall pipeline
	if (!ls_stall_counter && data_cache->state == WAIT_FILL_NOTIF) return;
	...
	if (!ls_stall_counter && data_cache->state == FILL_NOTIF_RECEIVED) {
		...
		data_cache->state = INSERT_CACHE_BLOCK;
		return;
	}
	...
	switch (op->opcode) {
		...
		case OP_LBU: {
			...
			if (op->opcode == OP_LW) {
				...
				if (op->opcode == OP_LH) val |= (val & 0x8000) ? 0xFFFF8000 : 0;
				...
				if (op->opcode == OP_LB) val |= (val & 0x80) ? 0xFFFFFF80 : 0;
...
void pipe_stage_execute() {
   ...
   */
	if (pipe.multiplier_stall > 0) pipe.multiplier_stall--;

	/* if downstream stall, return (and leave any input we had) */
	if (pipe.mem_op != NULL) return;

	/* if no op to execute, return */
	if (pipe.execute_op == NULL) return;
		...
		case OP_SPECIAL:
           ...
           */
				...
				case SUBOP_MFHI:
					...
					if (pipe.multiplier_stall > 0) return;
...
void pipe_stage_fetch() {
	...
	if (!fetch_stall_counter && instruction_cache->state == INSERT_CACHE_BLOCK) {
		//after 5 cycles, the retrieved cache block is inserted into the L2 cache and MSHR is freed.
		cache_insert(unified_l2_cache, pc);
		cache_free_mshr(unified_l2_cache, pc);
		// in the same cycle, if the pipline is stalled because of the cache block, l2 sends a fill notif to l1
		//insertion in the L1 cache
		cache_insert(instruction_cache, pc);
		//next cycle, pipeline unstalls
	...
	if (!fetch_stall_counter) {
		if (cache_request(instruction_cache, pipe.PC) == miss) {
