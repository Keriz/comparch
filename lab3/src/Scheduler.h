...
class Scheduler {
	...
	function<ReqIter(ReqIter, ReqIter)> compare[int(Type::MAX)] = {
	    ...
	    // ATLAS
	    /*
			1: TH—Over-threshold-requests-first: Requests that have been
			outstanding for more than T cycles in the memory controller are
			prioritized over all others (to prevent starvation for a long time).
			2: LAS—Higher-LAS-rank-thread-first: Requests from threads
			with higher LAS-based-rank are prioritized over requests from
			threads with lower LAS-based rank (to maximize system performance and preserve per-thread bank-level parallelism).
			3: RH—Row-hit-first: Row-hit requests are prioritized over rowconflict/closed requests (to exploit row buffer locality).
			4: FCFS—Oldest-first: Older requests are prioritized over
			younger requests. 
		*/
